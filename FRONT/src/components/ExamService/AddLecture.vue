  ...
  <div class="backdrop3">
    <div
      ...
      <div class=" h-9 flex items-center relative w-full p-3 pt-2 bg-primary  ">
        ...
        <img src="../../assets/Images/close-box.png" class="absolute right-3 cursor-pointer" @click="handle(false)"
          alt="" />
      </div>
      <div>
        <div class="flex justify-center space-y-2">
          <div>
            <form class="space-y-2">
              <div>
                <input type="text" id="name" dir="auto" v-model="name" name=""
                  class="rounded-lg  mx-3 my-1 w-48 text-right  px-2" />
                <label for="name"> :اسم المقرر</label>
              </div>
              <div>
                <input type="text" dir="auto" id="course_code" v-model="course_code" name=""
                  class="rounded-lg  mx-3 my-1 w-48  text-right  " />
        ...
        <div class="mx-auto">
          <button
            class="bg-primary text-body text-xl rounded-lg text-center m-4 py-1 px-2  hover:bg-hovercolor w-28  h-12"
            @click="add" v-if="proc == 'add'"> إضافة </button>
          <button
            class="bg-primary text-body text-xl rounded-lg text-center m-4 py-1 px-2  hover:bg-hovercolor w-28  h-12"
            @click="edit" v-if="proc == 'edit'"> تعديل </button>
        </div>
      </div>
    </div>
  </div>
