<template>
    <div class="backdrop3 "  >
     <div class="text-2xl text-primary flex space-y-4 rounded-lg flex-col my-52 border-2 w-9/12 bg-body mx-auto h-auto border-primary"    >
       <div class=" h-8 flex items-center relative w-full p-3 pt-2 bg-primary  "  >
 ...
 <thead>
     <th class="bg-primary text-white text-lg">تسجيل</th>
     <th class="bg-primary text-white text-lg" style="width:20%"> متكافئة</th>
     <th class="bg-primary text-white text-lg" style="width:20%"> عدد مرات الرسوب </th>
     <th class="bg-primary text-white text-lg">الفصل</th>
     <th class="bg-primary text-white text-lg">السنة</th>
     <th class="bg-primary text-white text-lg">المادة</th>
     <th class="bg-primary text-white text-lg">
     رمز المادة
     </th>
 </thead>
 <tbody>
...
<tr v-for="c in this.courses" :key="c.course_id">
   <td> <button class="bg-primary text-body text-xl rounded-lg text-center m-2 ml-56 py-2 px-2 hover:bg-hovercolor"   @click="addCourse(c.course_id,c.issimilar)"> اضافة</button>
</td>
   <td><input type="checkbox"  v-model="c.isSim"  /></td>
   <td class="text-primary">{{c.NumOfFail}}</td>
   <td>{{c.year_of_course}}</td>
   <td>{{c.semester}}</td>
   <td>{{c.course_name}}</td>
   <td>{{c.course_code}}</td>
